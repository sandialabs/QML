        ...
        if (!(W[i*n + j] != W[i*n + j])){
            double currentDist = currentDists[i*n + j];
                //for (k = 0; k < dimensionality; k++){
                    ...
                    if (scale < 0.5){ // 0.4 0.2 for existing tests
...
double optimizeC(double* N, double* W, unsigned dimensionality, unsigned n, double* errors, double* energies, double* maxEdgeErrors, unsigned dimReduceCutoff, double* distSum){
    ...
    printf("n: %u\n", n);
    bool errorBool = 0;
    double c0 = 0.0002;// 0.005  0.4/double(n); // 400 node 0.0005 //200 nodes 0.001 //0.000008  // 0.0001
    printf("c0: %lf\n", c0);
    double c1 = 1; // 0.01 // 0.5
    printf("c1: %lf\n", c1);
    double momentumConstant = 0.99; //0.8 // 0.999
...
double euclideanDistance(double *point1, double *point2, int dimensions) {
    ...
    for (int i = 0; i < dimensions; i += 8) { 
        double d0 = point1[i + 0] - point2[i + 0];
        double d1 = point1[i + 1] - point2[i + 1];
        double d2 = point1[i + 2] - point2[i + 2];
        double d3 = point1[i + 3] - point2[i + 3];
        double d4 = point1[i + 4] - point2[i + 4];
        double d5 = point1[i + 5] - point2[i + 5];
        double d6 = point1[i + 6] - point2[i + 6];
        double d7 = point1[i + 7] - point2[i + 7];
        dist0 += d0*d0;
        dist1 += d1*d1;
        dist2 += d2*d2;
        dist3 += d3*d3;
    ...
    distance = dist0 + dist1 + dist2 + dist3 + dist4 + dist5 + dist6 + dist7;
...
double averageDistanceToCluster(double *point, double *data, int numPoints, int dimensions, int *cluster, int clusterID, int pointID) {
    ...
    for (int i = 0; i < numPoints; i++) {
        if (i != pointID && cluster[i] == clusterID) {
            double distance = euclideanDistance(point, &data[i*dimensions], dimensions);
    ...
    // printf("ave dist after\n");
    // fflush(stdout);
    return distanceSum / count;
}

double silhouetteCoefficient(double *point, double *data, int numPoints, int dimensions, int *cluster, int pointID) {
    double a = averageDistanceToCluster(point, data, numPoints, dimensions, cluster, cluster[pointID], pointID);
    // printf("silhouette coef\n");
    // fflush(stdout);
    double b = INFINITY;
    for (int j = 0; j < numPoints; j++) {
        if (j != pointID && cluster[j] != cluster[pointID]) {
            double b_temp = averageDistanceToCluster(point, data, numPoints, dimensions, cluster, cluster[j], pointID);
            b = fmin(b, b_temp);
        }
    }

    // printf("silhouette coef after, %lf, %lf, %lf\n", a, b);
    // fflush(stdout);
    return (b - a) / fmax(a, b);
}

double silhouetteScore(double *data, int numPoints, int dimensions, int *cluster) {
    // printf("silhouette score\n");
    // fflush(stdout);
    double totalSilhouette = 0.0;

    for (int i = 0; i < numPoints; i++) {
        totalSilhouette += silhouetteCoefficient(&data[i*dimensions], data, numPoints, dimensions, cluster, i);
    }
    // printf("silhouette score after\n");
    // fflush(stdout);
    return totalSilhouette / numPoints;
}

void readClusterInfo(const char *filename, int *cluster, int numPoints) {
    // printf("file before\n");
    // fflush(stdout);
    ...
    for (int i = 0; i < numPoints; i++) {
        ...
        // Assuming the last column contains the cluster information
        token = strtok(buffer, ",");
        printf("strtok after, %s\n", token);
        fflush(stdout);
        char token2[3];
        token2[0] = token[0];
        token2[1] = token[1];
...
//    }
   ...
   readClusterInfo("data/bioGraphColored.csv", cluster, nActual);
   ...
   double* Ntmp;
   double* scores = (double*) calloc(dimensionality, sizeof(double));
   ...
   for (int i = dimensionality; i > 0; i -= 0){
      ...
      printf("embed after\n");
      fflush(stdout);
      double score = silhouetteScore(N, nActual, dimensionality, cluster);
      printf("Silhouette Score: %lf, %d\n", score, i);
      fflush(stdout);
      scores[i] = score;
      if (maxEdgeError == 0.0){
         maxEdgeError = fit;
    ...
    //   bool fitDim = score > maxEdgeError - 0.001;
      bool fitDim = fit < maxEdgeError * 1.1;
      if (fitDim){
         rangeTop = i;
         if (i < bestDim){
            memcpy(NSingleDimSave, N, n*dimensionality*sizeof(double));
            bestDim = i;
         }
      } else{
         memcpy(N, NSingleDimSave, n*dimensionality*sizeof(double));
         rangeBot = i;
      }
