...
# Fill in geodesic distance matrix
        ...
        for pn in range(nProp):
            for ki in range(nColl):
                # if (D[pt,peak_idxs[pn,ki]]==0) | (D[pt,peak_idxs[pn,ki]]>(pn+1)*dt):
                if (D[pt,peak_idxs[pn,ki]]==0) | (difDist > dist_idxs[pn,ki]) :
...
def perform_hamiltonian_test(qml_params):
    ...
    logeps_v = np.arange(-6,6,1)
    logh_v =  np.arange(-6,6,1)
...
if __name__ == '__main__':
    ...
    try:
        ...
        # run QML
        ids, distances, xScale = run(qml_params)
        ...
        ax[0].plot(distances[:,1], distances[:,0], label="QML")
        ...
        ax[0].legend()
        ax[1].plot(distances[:,1], difDists[:,0], label="QML")
